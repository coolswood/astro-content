...
export const GET: APIRoute = async ({ params }) => {
  ...
  try {
    ...
    console.error(
      `Error generating src/pages/api/mindhealth/[lang]/tests/burnout.json.ts:`,
      err,
    );
    throw new Error(
      `Failed to generate src/pages/api/mindhealth/[lang]/tests/burnout.json.ts: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
};
