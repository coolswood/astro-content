...
export const GET: APIRoute = async ({ params }) => {
  ...
  try {
    const daybookModules = import.meta.glob<{
      default: { daybookStart: DaybookTexts };
    ...
    const content = module.default.daybookStart;
    ...
    console.error(
      `Error generating src/pages/api/mindhealth/[lang]/texts/daybook/daybookStart.html.ts:`,
      err,
    );
    throw new Error(
      `Failed to generate src/pages/api/mindhealth/[lang]/texts/daybook/daybookStart.html.ts: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
};
