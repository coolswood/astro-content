// src/pages/api/[lang]/love_exercise.json.ts
// Миграция файла distortions/love_exercise по аналогии с Вашим start.json.ts.
// Минимально: без лишней логики; li() заменены на строковые <li>...</li>.
// Путь к локализации: src/i18n/${lang}/story/distortions/love_exercise.json.
// instagram в исходнике не используется — fallback не нужен.

import type { APIRoute } from 'astro';
import fs from 'fs/promises';
import path from 'path';
import { getLangStaticPaths } from '@/lib/getLangStaticPaths';
import { important, q } from '@/lib/storyHelper';

export const prerender = true;

export const getStaticPaths = getLangStaticPaths;

export const GET: APIRoute = async ({ params }) => {
  const lang = params.lang!;

  try {
    const story = JSON.parse(
      await fs.readFile(
        path.resolve(`src/i18n/${lang}/story/distortions/love_exercise.json`),
        'utf-8',
      ),
    );

    const output = {
      id: 'DISTORTIONS_LOVE_EXERCISE',
      color: '#BABFF3',
      url: 'distortions-love_exercise',
      title: story.title,
      description: story.description,
      time: 6,
      type: 'exercise',
      img: 'exercise',
      screens: [
        // Screen 1
        {
          __typename: 'ScreenText',
          steps: [
            `<h2>${story.title}</h2>`,
            `<p>${story.screen_1.texts[0]}</p>`,
            `<p>${story.screen_1.texts[1]}</p>`,
            `<p>${story.screen_1.texts[2]}</p>`,
            `<p>${story.screen_1.texts[3]}</p>`,
            `<p>${story.screen_1.texts[4]}</p>`,
            `<h2>${story.screen_1.texts[5]}</h2>`,
            `<p>${story.screen_1.texts[6]}</p>`,
            `<p>${story.screen_1.texts[7]}</p>`,
            `<p>${story.screen_1.texts[8]}</p>`,
            `<p>${story.screen_1.texts[9]}</p>`,
            q(story.screen_1.quote.text, story.screen_1.quote.author),
          ],
        },
        // Screen 2
        {
          __typename: 'ScreenText',
          steps: [
            `<h2>${story.screen_2.texts[0]}</h2>`,
            `<p>${story.screen_2.texts[1]}</p>`,
            `<p>${story.screen_2.texts[2]}</p>`,
            `<p>${story.screen_2.texts[3]}</p>`,
            `<li>${story.screen_2.texts[4]}</li>`,
            `<li>${story.screen_2.texts[5]}</li>`,
            `<li>${story.screen_2.texts[6]}</li>`,
            important(story.screen_2.texts[7]),
            `<li>${story.screen_2.texts[8]}</li>`,
            `<p>${story.screen_2.texts[9]}</p>`,
            `<p>${story.screen_2.texts[10]}</p>`,
            `<p>${story.screen_2.texts[11]}</p>`,
            q(story.screen_2.quote.text, story.screen_2.quote.author),
            `<p>${story.screen_2.texts[12]}</p>`,
            `<p>${story.screen_2.texts[13]}</p>`,
            `<p>${story.screen_2.texts[14]}</p>`,
            `<p>${story.screen_2.texts[15]}</p>`,
          ],
        },
        // Screen 3
        {
          __typename: 'ScreenText',
          steps: [
            `<h2>${story.screen_3.texts[0]}</h2>`,
            `<p>${story.screen_3.texts[1]}</p>`,
            `<p>${story.screen_3.texts[2]}</p>`,
            `<p>${story.screen_3.texts[3]}</p>`,
            `<p>${story.screen_3.texts[4]}</p>`,
            `<p>${story.screen_3.texts[5]}</p>`,
            `<li>${story.screen_3.texts[6]}</li>`,
            `<li>${story.screen_3.texts[7]}</li>`,
            `<li>${story.screen_3.texts[8]}</li>`,
            `<p>${story.screen_3.texts[9]}</p>`,
            `<li>${story.screen_3.texts[10]}</li>`,
            `<li>${story.screen_3.texts[11]}</li>`,
            `<li>${story.screen_3.texts[12]}</li>`,
            `<p>${story.screen_3.texts[13]}</p>`,
            `<p>${story.screen_3.texts[14]}</p>`,
            `<li>${story.screen_3.texts[15]}</li>`,
            `<li>${story.screen_3.texts[16]}</li>`,
            `<li>${story.screen_3.texts[17]}</li>`,
            `<p>${story.screen_3.texts[18]}</p>`,
            `<p>${story.screen_3.texts[19]}</p>`,
            `<p>${story.screen_3.texts[20]}</p>`,
            `<p>${story.screen_3.texts[21]}</p>`,
            important(story.screen_3.texts[22]),
            `<p>${story.screen_3.texts[23]}</p>`,
            `<p>${story.screen_3.texts[24]}</p>`,
            `<p>${story.screen_3.texts[25]}</p>`,
            `<p>${story.screen_3.texts[26]}</p>`,
            `<p>${story.screen_3.texts[27]}</p>`,
          ],
        },
      ],
    };

    return new Response(JSON.stringify(output), {
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (err) {
    console.error(
      `Error generating src/pages/api/mindhealth/[lang]/story/distortions/distortions_love_exercise.json.ts:`,
      err,
    );
    throw new Error(
      `Failed to generate src/pages/api/mindhealth/[lang]/story/distortions/distortions_love_exercise.json.ts: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
};
