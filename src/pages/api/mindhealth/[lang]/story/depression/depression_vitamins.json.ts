// src/pages/api/[lang]/vitamins.json.ts
// Миграция файла depression_vitamins (vitamins) по аналогии с Вашим start.json.ts.
// Минимально: без лишней логики; li() заменены на строковые <li>...</li>.
// Путь к локализации: src/i18n/${lang}/story/depression/vitamins.json.
...
export const GET: APIRoute = async ({ params }) => {
  ...
  try {
    const story = JSON.parse(
      await fs.readFile(
        path.resolve(`src/i18n/${lang}/story/depression/vitamins.json`),
        'utf-8',
      ),
    );

    const output = {
      id: 'DEPRESSION_VITAMINS',
      color: '#FFBFBF',
      url: 'depression-vitamins',
      title: story.title,
      description: story.description,
      time: 6,
      type: 'theory',
      img: 'depression_vitamins',
      screens: [
        {
          // @ts-ignore
        ...
        },
        {
          ...
          steps: [
            ...
            important(story.screen_4.texts[4]),
    ...
    console.error(
      `Error generating src/pages/api/mindhealth/[lang]/story/depression/depression_vitamins.json.ts:`,
      err,
    );
    throw new Error(
      `Failed to generate src/pages/api/mindhealth/[lang]/story/depression/depression_vitamins.json.ts: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
};
