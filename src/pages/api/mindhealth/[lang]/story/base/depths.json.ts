...
// Миграция файла depths по аналогии с Вашим start.json.ts (минимально; без лишней логики).

import type { APIRoute } from 'astro';
import fs from 'fs/promises';
import path from 'path';
import { getLangStaticPaths } from '@/lib/getLangStaticPaths';
import { instagramStep, step, important } from '@/lib/storyHelper';
...
export const GET: APIRoute = async ({ params }) => {
  ...
  try {
    ...
    // Получаем common для stepper (хотя в depths они не используются)
    ...
    throw new Error(
      `Failed to generate src/pages/api/mindhealth/[lang]/story/base/depths.json.ts: ${
