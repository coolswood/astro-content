...
- **Конвейер**: контент — 4 стадии main → editor → review → fix («критикуй
  отдельно, правь отдельно»); keys/ui — 3 стадии main → editor → tech.
  После перевода запускается **коллегия** (`scripts/lib/judge.ts`): судит
  переведённые листья (issues → deliberate → recommend), подтверждённые
  рекомендации вписывает механически и пересматривает локаль — до раунда
  без правок (не более 3; `--no-judge` отключает). Осцилляции (возврат листа
  к бывшему значению) и потеря тегов блокируются. Отчёты —
  `scripts/qa/reports/<ts>-judge-<файл>/`. Промпты
