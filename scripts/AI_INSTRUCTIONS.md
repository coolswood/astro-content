...
score/verdict/summary). Отчёт — `scripts/qa/reports/<ts>-audit-<label>/`.
Отличие от слепого парного судьи (`qa/eval.ts`): тот сравнивает ДВА
перевода между собой, аудит разбирает ОДИН перевод и выдаёт готовые
рекомендации «что и как заменить».
