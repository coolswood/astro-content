...
bun scripts/gemini-simple-bot.ts story/automatic.json de --provider chatgpt
bun scripts/gemini-ui-simple-bot.ts app_interface.json de --provider chatgpt

# gemini-translate-keys: мультиязычный перевод интерфейса батчами по 3 языка.
# По умолчанию прогоняет все 3 этапа (перевод → редактура → тех-аудит) на каждом
# батче, результаты кэшируются в scripts/.translate-cache для resume при сбое.
bun scripts/gemini-translate-keys.ts --provider chatgpt
# Опции:
