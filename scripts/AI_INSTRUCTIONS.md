...
- **--main-path-map** (эксперимент, `StageRequest`/`runPipeline`): main отвечает
  плоской картой «путь → перевод» по явному списку путей — сдвиг содержимого
  между соседними листьями («крючки») структурно невозможен, полнота
  проверяется по путям на месте, recovery почти не нужен. Тест на
  de/perfectionism: слепой win-rate 60% против документного формата, стадии
  ~40% быстрее (jsonMode с первой попытки), потерь листьев ноль. Пока флаг
  выключен по умолчанию — расширение на корпус локалей перед дефолтом.
